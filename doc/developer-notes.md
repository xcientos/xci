...
- [Development guidelines](#development-guidelines)
    - [General XCIENTOS Core](#general-xcientos-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
XCIENTOS Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play XCIs (tXCI)" on the test network, if you
...
XCIENTOS Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/xcientos-project/xcientos/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_xcientos
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_xcientos --log_level=test_suite
$ valgrind -v --leak-check=full src/xcientosd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_xcientos.coverage/index.html`.
```
...
- BitcoinMiner : Generates XCIs (if wallet is enabled).
...
However, in open source software such as XCIENTOS Core, where everyone uses
...
pay attention to for reviewers of XCIENTOS Core code.

General XCIENTOS Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: XCIENTOS Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against XCIENTOS Core so that they can be integrated
...
file descriptors it will cause problems with XCIENTOS's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:XCIENTOS-Project/XCIENTOS.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `xcientos-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `xcientos-cli` and the GUI debug console use this table to determine how to
