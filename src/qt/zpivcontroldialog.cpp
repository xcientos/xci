    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zXCI.";
            else if (!mint.isSeedCorrect)
                strReason = "The zXCI seed used to mint this zXCI is not the same as currently hold in the wallet";
            else
