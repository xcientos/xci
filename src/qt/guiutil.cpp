...
// Copyright (c) 2015-2018 The XCIENTOS developers
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a XCIENTOS address (e.g. %1)").arg("XQCVBSRsUWWHQHquWKJSvZPttbXrWQRtzT"));
...
{
    // return if URI is not valid or is no XCIENTOS: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::XCI, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert xcientos:// to xcientos:
    //
    //    Cannot handle this later, because xcientos:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::XCI, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open xcientos.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "XCIENTOS.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for XCIENTOS.lnk
...
{
    return GetAutostartDir() / "xcientos.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a xcientos.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=XCIENTOS\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the xcientos app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add xcientos app to startup item list
