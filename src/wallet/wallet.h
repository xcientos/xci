...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 1000000 XCI at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zXCI send
enum ZerocoinSpendStatus {
    ZXCI_SPEND_OKAY = 0,                            // No error
    ZXCI_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZXCI_WALLET_LOCKED = 2,                         // Wallet was locked
    ZXCI_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZXCI_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZXCI_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZXCI_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZXCI_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZXCI_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZXCI_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZXCI_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZXCI_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZXCI_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZXCI_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZXCI_SPENT_USED_ZXCI = 14,                      // Coin has already been spend
    ZXCI_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZXCI_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZXCIOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzXCITracker> zpivTracker;
    ...
    void setZWallet(CzXCIWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzXCITracker>(new CzXCITracker(strWalletFile));
    }

    CzXCIWallet* getZWallet() { return zwalletMain; }
    ...
    /** zXCI reset */
    boost::signals2::signal<void()> NotifyzXCIReset;
