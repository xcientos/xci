...
public:
    ...
    bool WriteZXCISeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZXCISeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZXCISeed_deprecated(uint256& seed);
    bool EraseZXCISeed();
    bool EraseZXCISeed_deprecated();

    bool WriteZXCICount(const uint32_t& nCount);
    bool ReadZXCICount(uint32_t& nCount);
