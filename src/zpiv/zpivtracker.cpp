// Copyright (c) 2018-2019 The XCIENTOS developers
...
CzXCITracker::CzXCITracker(std::string strWalletFile)
{
...
CzXCITracker::~CzXCITracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzXCITracker::Init()
{
...
bool CzXCITracker::Archive(CMintMeta& meta)
{
...
bool CzXCITracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzXCITracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzXCITracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzXCITracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzXCITracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzXCITracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzXCITracker::GetSerialHashes()
{
...
CAmount CzXCITracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzXCITracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzXCITracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzXCITracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzXCITracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzXCITracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzXCITracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzXCITracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzXCITracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzXCITracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzXCITracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzXCIWallet* zXCIWallet)
{
    bool iszXCIWalletInitialized = (NULL != zXCIWallet);
    CMintMeta meta;
    ...
    if (! iszXCIWalletInitialized)
        zXCIWallet = new CzXCIWallet(strWalletFile);
    meta.isSeedCorrect = zXCIWallet->CheckSeed(dMint);
    if (! iszXCIWalletInitialized)
        delete zXCIWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzXCITracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzXCITracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzXCITracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzXCITracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzXCITracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzXCITracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzXCIWallet* zXCIWallet = new CzXCIWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zXCIWallet);
        }
        delete zXCIWallet;
