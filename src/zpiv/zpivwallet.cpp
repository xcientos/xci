...
CzXCIWallet::CzXCIWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZXCISeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZXCISeed_deprecated()) {
                    LogPrintf("%s: Updated zXCI seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzXCIWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZXCICount(nCountLastUsed);
    else if (!walletdb.ReadZXCICount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzXCIWallet::Lock()
{
    seedMaster = 0;
}

void CzXCIWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzXCIWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZXCI(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzXCIWallet::LoadMintPoolFromDB()
{
...
void CzXCIWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzXCIWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzXCIWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzXCIWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZXCI(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzXCIWallet::SeedToZXCI(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzXCIWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzXCIWallet::GenerateDeterministicZXCI(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzXCIWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzXCIWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzXCIWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
